...
from school_management_app.models import Students, Courses, Subjects, CustomUser, Attendance, AttendanceReport, StudentResult, SessionYearModel, News, SComment, Parents, FeedBackParents, NotificationParents, PNews, PComment, FinancialRecord
...
def parent_view_financial_record_list(request):
    parent=Parents.objects.get(admin=request.user.id)
    parent_student=parent.student_id
    record=FinancialRecord.objects.filter(student_id=parent_student)
    user=CustomUser.objects.get(id=request.user.id)
    parent=Parents.objects.get(admin=user)
    parent_notifcation=Parents.objects.get(admin=request.user.id)
    notifications=NotificationParents.objects.filter(parent_id=parent_notifcation.id)
    
    return render(request,"parent_template/fee.html",{"notifications":notifications,"parent":parent, "record":record})
