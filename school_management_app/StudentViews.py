...
from school_management_app.models import Students, Courses, Subjects, CustomUser, Attendance, AttendanceReport, \
    LeaveReportStudent, FeedBackStudent, NotificationStudent, StudentResult, SessionYearModel, News, SComment, FinancialRecord
...
def student_view_financial_record_list(request):
    student=Students.objects.get(admin=request.user.id)
    record=FinancialRecord.objects.filter(student_id=student.id)
    notifications=NotificationStudent.objects.filter(student_id=student.id)
    user=CustomUser.objects.get(id=request.user.id)
    student=Students.objects.get(admin=user)
    return render(request,"student_template/fee.html",{"notifications":notifications,"student":student, "record":record})
